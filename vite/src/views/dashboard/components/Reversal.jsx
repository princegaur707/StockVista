...
const ReversalTable = ({ setSymbolToken, updateToken, liveMarketData }) => {
  ...
  const fetchReportData = async (retries = 3, delay = 1000) => {
    try {
      const response = await requestWithToken(`${import.meta.env.VITE_API_URL}/api/service/reversal-data`);
  ...
  const columns = [
    ...
    {
      field: 'pct_change',
      headerName: 'Reversal Performance',
      flex: 1,
      headerAlign: 'left',
      type: 'number',
      renderCell: (params) => {
