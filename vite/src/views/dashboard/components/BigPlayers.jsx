...
const BigPlayersTable = ({ setSymbolToken, updateToken, liveMarketData }) => {
  ...
  const fetchReportData = async (retries = 3, delay = 1000) => {
    try {
      const response = await requestWithToken(`${import.meta.env.VITE_API_URL}/api/service/big-players`);
  ...
  const columns = [
    ...
    {
      field: 'pct_change',
      headerName: 'Big Players Performance',
      flex: 1,
      headerAlign: 'left',
      type: 'number',
      renderCell: (params) => {
...
export default BigPlayersTable;
